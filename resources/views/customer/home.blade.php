...
<div class="screen" style="background:#F5F7FA; min-height:100vh; display:block !important;">
  ...
  <div style="background:linear-gradient(180deg, #0EA5E9 0%, #0284C7 100%); padding:12px 16px 20px; border-radius:0 0 20px 20px; position:relative; z-index:100;">
    ...
    <div style="margin-bottom:14px; position:relative; z-index:101;" id="home-search-wrapper">
      <form action="{{ url('/search') }}" method="GET" style="margin:0;" id="home-search-form">
        <div style="background:#fff; border-radius:12px; padding:8px 12px; display:flex; align-items:center; gap:8px; box-shadow:0 2px 8px rgba(0,0,0,0.08); border:1px solid rgba(255,255,255,0.4); cursor:pointer;">
          ...
          <input 
            ...
            autocomplete="off"
            style="flex:1; border:none; outline:none; font-size:14px; color:#1A1A1A; font-weight:500; background:transparent; padding:6px 0; cursor:pointer;"
          >
...
function redirectToSearchPage() {
  ...
  // Live Instant Medicine Search Autocomplete on Homepage
  (function() {
    ...
    function fetchSuggestions() {
      ...
      debounceTimer = setTimeout(() => {
        fetch("{{ url('/medicines/search') }}?q=" + encodeURIComponent(query.trim()))
          .then(res => res.json())
          .then(data => {
            if (!data || !Array.isArray(data) || data.length === 0) {
              suggestionsBox.innerHTML = `
                <div style="padding:14px 16px; text-align:center; color:#64748B; font-size:13px;">
                  <span>🔍</span> Koi medicine nahi mili "<strong>${escapeHtml(query.trim())}</strong>" ke naam se.
                </div>`;
            } else {
              let html = '';
              data.forEach(med => {
              ...
              html += `
                <a href="{{ url('/search') }}?q=${encodeURIComponent(query.trim())}" style="display:block; text-align:center; padding:10px 14px; background:#F0F9FF; color:#0EA5E9; font-size:13px; font-weight:700; text-decoration:none; border-radius:0 0 14px 14px;">
                  Sabhi "${escapeHtml(query.trim())}" results dekhein →
                </a>`;
