...
function redirectToSearchPage() {
  ...
  // Instant redirect to dedicated search page when typing or focusing search input on homepage
  (function() {
    ...
    function goToSearchPage(qVal) {
      const q = qVal || searchInput.value || '';
      window.location.href = "{{ url('/search') }}" + (q ? "?q=" + encodeURIComponent(q) : "");
    }

    searchInput.addEventListener('input', function() {
      if (this.value.trim().length >= 1) {
        goToSearchPage(this.value.trim());
      }
    });
